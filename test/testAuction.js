...
describe("Auction functionality", function () {
  ...
  beforeEach(async () => {
    ...
    await storageContract
      .connect(owner)
      .depositNft(nftContract.address, iterator);
    await storageContract
      .connect(owner)
      .createFraction(
        nftContract.address,
    ...
    fractionAddress = await storageContract.getFractionAddressFromNft(
      nftContract.address,
      iterator
    );
  ...
  it("should be able to let an address stake their tokens into the auction contract", async () => {
    ...
    let balanceOfAuction = await fractionInstance.balanceOf(
  ...
  it("should be able to let an address withdraw their tokens after they stake it in the auction contract", async () => {
    ...
    balanceOfAuction = await fractionInstance.balanceOf(
      auctionContract.address
    );
  ...
  it("SHOULD NOT be able to withdraw more tokens than what is staked by address", async () => {
    ...
    let balanceOfAuctionContract = await fractionInstance.balanceOf(
      auctionContract.address
    );
  ...
  it("should allow a user to start a proposal with an NFT they deposited", async () => {
    ...
    await auctionContract
      .connect(owner)
      .startProposal(nftContract.address, iterator, 2, 20, 1);

    let isNftInProposal = await auctionContract.getIsNftInProposal(
      nftContract.address,
      iterator
    );
    ...
    let proposalStartPrice = await auctionContract.getProposalStartPrice(
      nftContract.address,
      iterator
    );
    ...
    let proposalInitiator = await auctionContract.getProposalInitiator(
      nftContract.address,
      iterator
    );
    ...
    let proposalFinishTime = await auctionContract.getProposalFinishTime(
      nftContract.address,
      iterator
    );
    ...
    let proposalTotalVoted = await auctionContract.getProposalTotalVoted(
      nftContract.address,
      iterator
    );
    ...
    let proposalScore = await auctionContract.getProposalScore(
      nftContract.address,
      iterator
    );
  ...
  it("SHOULD NOT let you create a proposal of an NFT you do not own", async () => {
    ...
    try {
      ...
      await console.log(
        "address could not start a proposal of an NFT they do not own"
      );
    }
  });
  ...
  it("should let an address vote on the proposal if they own tokens", async () => {
    ...
    let ownerBalance = await auctionContract.getAddressTokenBalance(
      fractionAddress,
      owner.address
    );
    ...
    await auctionContract
      .connect(owner)
      .startProposal(nftContract.address, iterator, 2, 20, ownerBalance);

    let score1 = await auctionContract.getProposalScore(
      nftContract.address,
      iterator
    );
    let totalVoted = await auctionContract.getProposalTotalVoted(
      nftContract.address,
      iterator
    );

    expect(score1).to.equal(ownerBalance);
    expect(totalVoted).to.equal(ownerBalance);

    await auctionContract
      .connect(owner)
      .voteOnProposal(nftContract.address, iterator, false, 1);

    let score2 = await auctionContract.getProposalScore(
      nftContract.address,
      iterator
    );
    let expectScore = (await score1) - 1;

    expect(score2).to.equal(expectScore);

    //testing enviornment bug. score1 + 1 would add 900 instead of 1
    let expectTotalVoted = await score1;
    await expectTotalVoted++;

    totalVoted = await auctionContract.getProposalTotalVoted(
      nftContract.address,
      iterator
    );
    expect(totalVoted).to.equal(expectTotalVoted);
  });
  ...
  it("should be able to start an auction when 50% of tokens vote yes for an auction", async () => {
    ...
    let isNftInAuction = await auctionContract.getIfNftIsInAuction(
      nftContract.address,
      iterator
    );
  ...
  it("should reset all proposal variables when an auction starts", async () => {
    ...
    let score = await auctionContract.getProposalScore(
      nftContract.address,
      iterator
    );
    expect(score).to.equal(0);

    let nftProposalStartPrice = await auctionContract.getProposalStartPrice(
      nftContract.address,
      iterator
    );
  ...
  it("should be able to bid on an auction", async () => {
    ...
    let currentBid = await auctionContract.getAuctionCurrentBid(
      nftContract.address,
      iterator
    );
    ...
    await expectRevert(
      auctionContract
        .connect(addr1)
        .bidOnAuction(nftContract.address, iterator, { value: 1 }),
    ...
    let auctionLeader = await auctionContract.getAuctionCurrentLeader(
      nftContract.address,
      iterator
    );
    let auctionLeadingBid = await auctionContract.getAuctionCurrentBid(
      nftContract.address,
      iterator
    );
  ...
  it("should allow the auction winner to withdraw the NFT", async () => {
    ...
    let isFractionalised = await storageContract.getIsNftFractionalised(
      nftContract.address,
      iterator
    );
  ...
  it("should give fraction token of a finish auction the right amount when claiming", async () => {
    ...
    //
    await auctionContract.connect(owner).claimFromBuyoutTokens(fractionAddress);
    //
    let balanceAfter = await prov.getBalance(owner.address);
    await console.log("Balance after: " + balanceAfter);
  });
});
