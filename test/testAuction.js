    ...
    beforeEach(async () => {
       ...
       await storageContract.connect(owner).createFraction(nftContract.address, iterator, "tokenName", "TK", 8000, 8);
        ...
        it ('should let an address vote on the proposal if they own tokens', async() => {
            ...
            let ownerBalance = await auctionContract.getAddressTokenBalance(fractionAddress, owner.address);

            await auctionContract.connect(owner).startProposal(nftContract.address, iterator, 2, 20, ownerBalance);

            let score1 = await auctionContract.getProposalScore(nftContract.address, iterator);
            ...
            let score2 = await auctionContract.getProposalScore(nftContract.address, iterator);
